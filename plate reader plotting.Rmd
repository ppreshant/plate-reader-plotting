---
title: "Plate reader plotting : S022"
author: "Prashant"
date: "Nov 26, 2019"
...
Plotting dose response by day

```{r plotting9_pInt8 only}
sel_table11 <- all_days_table %>% group_and_summarize_at('GFP/OD') %>% filter(str_detect(Samples, 'pInt|pRV')) %>% ungroup() %>%  mutate(Inducer = as.numeric(Inducer)) %>% 
...
hill_table11 <- sel_table11 %>% group_by(Samples) %>% rename(L = Inducer, y = mean) %>% 
  ...
  mutate (fit = map(data, hill_fit), y.fit = map(fit, augment)) %>% # call hill fitting function on a single time point data
  unnest(y.fit)

  
sel_table11.fit  <- sel_table11 %>% mutate(y.fit = hill_table11$y.fit) # take the fit curve for plotting

plt11 <- plot_dose_response(sel_table11.fit) + geom_line(aes(x = Inducer, y = y.fit), linetype = 2)
