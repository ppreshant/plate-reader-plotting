...
Plotting all data interactive plotly

```{r plotting0}
sel_table1 <- merged2 %>% group_and_summarize_at(c('OD', 'GFP', 'RFP', 'GFP/OD', 'RFP/OD'), Samples, Dilution)
plt1_OD <- ggplot(sel_table1, aes(x = Dilution, y = mean, colour = Samples)) + geom_point() + geom_line() + facet_wrap(~Measurement, scales = 'free')
plt1_OD %>% format_classic() %>% ggplotly()
```

Plotting all data interactive plotly zoom in

```{r plotting0zoom}
sel_table1 <- merged2 %>% group_and_summarize_at(c('OD', 'GFP', 'RFP', 'GFP/OD', 'RFP/OD'), Samples, Dilution)
plt1_OD <- ggplot(sel_table1, aes(x = Dilution, y = mean, colour = Samples)) + geom_point() + geom_line() + facet_wrap(~Measurement, scales = 'free') + xlim(c(0,1))
plt1_OD %>% format_classic() %>% ggplotly()
```
...
plt1_OD <- ggplot(sel_table1_OD, aes(x = Dilution, y = mean)) + geom_point() + geom_line() + ylab('OD')
plt1_OD
```
...
```{r plotting2GFP}
sel_table1_OD <- merged2 %>% filter(str_detect(Samples, 'fG')) %>% group_and_summarize_at('GFP', Dilution)
plt1_OD <- ggplot(sel_table1_OD, aes(x = Dilution, y = mean)) + geom_point() + geom_line()+ ylab('GFP')
plt1_OD
```
]
Plotting GFP/OD fGFP

```{r plotting3GFP/od}
sel_table1_OD <- merged2 %>% filter(str_detect(Samples, 'fG')) %>% group_and_summarize_at('GFP/OD', Dilution)
plt1_OD <- ggplot(sel_table1_OD, aes(x = Dilution, y = mean)) + geom_point() + geom_line() + ylab('GFP/OD')
plt1_OD
```

```{r plotting4od}
sel_table1 <- merged2 %>% group_and_summarize_at('OD', Samples, Dilution)
plt1_OD <- ggplot(sel_table1, aes(x = Dilution, y = mean, colour = Samples)) + geom_point() + geom_line() + ylab('OD')
plt1_OD
```
