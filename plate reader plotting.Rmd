...
flnm <- '../S013_pPK7.2 RBS lib sel triplicate_1-6-19_2.xlsx'
...
merged4 <- merged3 %>% ungroup() %>%  mutate(Samples = fct_relevel(merged3$Samples, 'pRV01'))
```
...
```{r plotting, echo=T}
merged4 %>% ggplot(aes(Samples, mean, shape = Inducer, fill = Inducer)) + geom_bar(stat= 'identity', position = 'dodge') + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), position = 'dodge') + scale_shape_manual(values = c(1,16)) -> plt
