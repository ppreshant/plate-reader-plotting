...
```{r plotting5}
sel_table1 <- all_days_table %>% group_and_summarize_at('GFP') %>% filter(!str_detect(Samples, "MG|LB|SH|GFP")) %>% filter(!str_detect(Inducer, "s")) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column
