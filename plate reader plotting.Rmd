...
```{r plotting, echo=T}
sel_table1 <- all_days_table %>% filter(!str_detect(Samples, "MG")) %>% filter(!str_detect(Inducer, "_"))
sel_table1$Time %<>% as.numeric()
plt1 <- ggplot(sel_table1, aes(Time, mean, colour = Inducer)) + geom_point(size = 1) +  geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.25) + facet_wrap(~ Samples) + geom_line() + scale_x_continuous(breaks = c(0,6,24)) + ylab('GFP/RFP') + ggtitle('AHL flipping with time')
