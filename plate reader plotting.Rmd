...
plt1 <- plot_mean_facetted(sel_table2_s) + ylab('Fold') + geom_hline(yintercept = WT_baseline, linetype = 2, alpha = .5) +  geom_hline(yintercept = rGFP_baseline, linetype = 2, alpha = .5) # plot the mean, add y axis label and a baseline indication
...
# Selecting top 5 data points + controls
sel_table6_s <- sel_table6 %>% filter(str_detect(Time, '25')) %>% ungroup() %>% mutate(category = ifelse(str_detect(category, 'pPK'), 'Mutants', as.character(category))) %>% mutate(category = fct_infreq(category), Inducer = '1 uM') %>% group_by(Samples, Inducer, category, Time) %>% filter(str_detect(Samples, 'RV01 $|rG|MG|7_21|7_6|7_28|7_32|7_17')) 
