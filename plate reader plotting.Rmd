...
```{r plotting9_pInt8 only}
sel_table11 <- all_days_table %>% group_and_summarize_at('GFP/OD') %>% filter(str_detect(Samples, 'pInt|pRV')) %>% ungroup() %>%  mutate(Inducer = as.numeric(Inducer)) %>% filter(Time == 1) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column

plt11 <- plot_dose_response(sel_table11)
