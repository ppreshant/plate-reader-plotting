...
all_days_table <- merged2 %>% separate(Samples, into = c('category', NA), sep = '\\_|\\+', remove = F) %>% mutate(Samples  = str_replace(Samples, '(.*)\\_(.*)', '\\2')) # create a category row for controls and the library clones 6 and 7
