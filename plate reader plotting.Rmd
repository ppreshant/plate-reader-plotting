...
merged2$Samples %<>% fct_relevel(plotting_order) # change sample order: in ascending order of GFP/OD
```
...
grouped3 %>% ggplot(aes(Samples, mean, color = Measurement, shape = Inducer)) + geom_point(size = 2) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.05) + scale_shape_manual(values = c(1,19)) + ggtitle('AHL influence on RFP') + ylab('OD 600') -> plt
plt2 <- plt %>% format_classic(.) %>% format_logscale()
plt2
# ggsave('plots and data/S010_faceted.png', height = 3.5, width = 14)
```
