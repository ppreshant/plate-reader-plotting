...
all_days_table %<>% mutate(category = if_else(Inducer == 1e-3, 'Control', category)) # moving Glucose into Control - to avoid going into hill function fitting 
all_days_table$Reporter %<>% fct_relevel('rGFP', "pEMF02", "pSH02", "fGFP", "DH10B") # Change order of plotting reporters

dose_response_data <- all_days_table %>% group_and_summarize_at('GFP/OD') %>% filter(!str_detect(Time, "0")) # exclude overnight data and take mean and SD for GFP/OD
```
## Data tables
...
Non linear regression to fit data to hill function

```{r hill_fits}
hill_table2 <- all_days_table %>% filter(!str_detect(Time, "0"), !str_detect(category, 'Control') | str_detect(Reporter, 'pSH02')) %>%        # select relevant data for hill fits - excluding controls without inducer doses
  group_by(category, Reporter) %>% rename(L = Inducer, y = 'GFP/OD') %>% 
...
sel_table2 <- all_days_table %>% group_and_summarize_at('GFP/OD') %>% filter(!str_detect(Time, "0"))
