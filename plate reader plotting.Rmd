...
sel_table5 <- all_days_table %>% group_and_summarize_at('GFP/RFP') %>% filter(!str_detect(Inducer, "s")) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column

plt5b <- plot_time_series(sel_table1, x_breaks = sheet_times, y_axis_label = 'GFP/RFP (a.u.)')
...
```{r plotting6_uninduced_zoomed, fig.width = 5, fig.height = 3.5}
...
```{r plotting6_uninduced_zoomed_select, fig.width = 5, fig.height = 3.5}
# Extract desired data: exclude pPK7 mutants
sel_uninduced_zoom_select <- sel_table6_uninduced_zoom %>% filter(str_detect(Samples, 'pRV|pInt|Controls')) 

plt6_sel_zoom <- plot_time_series(sel_uninduced_zoom_select, x_breaks = sheet_times)
plt6_sel_zoom
```
