...
Plotting all GFP and RFPs (individual replicates)
...
sel_table2$Samples %<>% fct_relevel(levels(.)[c(6,5,4,7,8,1)]) # manually reorder sample order for plotting

plt3 <- ggplot(sel_table2, aes(Time, Reading, colour = Measurement, shape = Inducer, group = interaction(Inducer, Measurement, `Replicate #`))) + geom_line() + geom_point(size = 2) + scale_shape_manual(values = c(1,19)) + facet_wrap(~Samples, scales = 'fixed') +  scale_x_continuous(breaks = c(0,6,24)) + ylab('GFP/RFP') + ggtitle('AHL flipping with time') 
...
Plotting mean and variance of GFP/OD and RFP/OD

```{r plotting, echo=T}
sel_table1 <- all_days_table %>% summarize_and_arrange_all() %>% filter(!str_detect(Samples, "MG|LB")) %>% filter(!str_detect(Inducer, "s")) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column
sel_table1$Samples %<>% fct_relevel(levels(.)[c(5,4,1,3,2,6)]) # manually reorder sample order for plotting
