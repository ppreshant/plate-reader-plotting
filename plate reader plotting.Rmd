...
plt_GFP <- ggplot(data = gfp_order_processed.data,
  ...
  {plt_GFP + geom_text(data = processed.data %>% select(ends_with('mean')) %>% unique(),  
...
long_fluor.normalized_processed <- processed.data %>% 
  select(matches('/OD')) %>%  # choose OD, x/OD and _bs and _mean additions
  ...
  pivot_longer(cols = c(-1, -2, -3), # pull measurement types and summary type (mean vs 'raw' value) into two cols
