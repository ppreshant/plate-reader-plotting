...
```{r plotting2all, fig.width = 5, fig.height = 3.5}
# Extract desired data: only pRV01 - various days
sel_table_uninduced <- all_days_table %>% group_and_summarize_at('GFP/OD') %>%  filter(str_detect(Inducer,'^0'))

plt2 <- ggplot(sel_table_uninduced, aes(Time, mean, colour = Reporter, shape = Inducer)) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 1) + geom_line() + geom_point(size = 2, fill = 'white') + scale_shape_manual(values = c(21,19)) + facet_wrap(~ Samples, ncol = 2) + ggtitle('Un-induced flipping with time') + ylab('GFP/OD (a.u.)') + scale_x_continuous(breaks = c(0,6,24,48)) # + geom_hline(yintercept = WT_baseline, linetype = 2) +  geom_hline(yintercept = rGFP_baseline, linetype = 2) # plot the mean, add y axis label and a baseline indication
...
# Extract desired data: only pRV01 - various days
sel_table_uninduced_2 <- all_days_table %>% group_and_summarize_at('GFP/OD') %>%  filter(str_detect(Inducer,'^0')) %>% filter(str_detect(Samples, 'RV01|pInt|Control')) 

plt2 <- ggplot(sel_table_uninduced_2, aes(Time, mean, colour = Reporter, shape = Inducer)) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 1) + geom_line() + geom_point(size = 2, fill = 'white') + scale_shape_manual(values = c(21,19)) + facet_wrap(~ Samples, ncol = 2) + ggtitle('Un-induced flipping with time') + ylab('GFP/OD (a.u.)') + scale_x_continuous(breaks = c(0,6,24,48)) # + geom_hline(yintercept = WT_baseline, linetype = 2) +  geom_hline(yintercept = rGFP_baseline, linetype = 2) # plot the mean, add y axis label and a baseline indication
...
# Extract desired data: only pRV01 - various days
sel_table_uninduced_2_zoom <- all_days_table %>% group_and_summarize_at('GFP/OD') %>%  filter(str_detect(Inducer,'^0') & str_detect(Samples, 'pRV|pPK|Controls') & !str_detect(Reporter, 'fGFP')) 

plt2z <- ggplot(sel_table_uninduced_2_zoom, aes(Time, mean, colour = Reporter, shape = Inducer)) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 1) + geom_line() + geom_point(size = 2, fill = 'white') + scale_shape_manual(values = c(21,19)) + facet_wrap(~ Samples, ncol = 2) + ggtitle('Un-induced flipping with time') + ylab('GFP/OD (a.u.)') + scale_x_continuous(breaks = c(0,6,24,48)) # + geom_hline(yintercept = WT_baseline, linetype = 2) +  geom_hline(yintercept = rGFP_baseline, linetype = 2) # plot the mean, add y axis label and a baseline indication

plt2za <- plt2z %>% format_classic() 
plt2za# + ylim(0, 2000)
```

Uninduced pRV01 and pInt8, zoomed in (excluding fGFP positive control)

```{r plotting_uninduced_zoomed_all, fig.width = 5, fig.height = 3.5}
# Extract desired data: only pRV01 - various days
sel_uninduced_zoom_select <- sel_table_uninduced_2_zoom %>% filter(str_detect(Samples, 'pRV|pInt|Controls')) 

plt2z <- ggplot(sel_uninduced_zoom_select, aes(Time, mean, colour = Reporter, shape = Inducer)) + geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 1) + geom_line() + geom_point(size = 2, fill = 'white') + scale_shape_manual(values = c(21,19)) + facet_wrap(~ Samples, ncol = 2) + ggtitle('Un-induced flipping with time') + ylab('GFP/OD (a.u.)') + scale_x_continuous(breaks = c(0,6,24,48)) # + geom_hline(yintercept = WT_baseline, linetype = 2) +  geom_hline(yintercept = rGFP_baseline, linetype = 2) # plot the mean, add y axis label and a baseline indication
...
Plotting only WT pRV01 and pSH01 : GFP/OD : linear scale
