...
plt1a + ylim(0, 2500)
ggsave('plots and data/S018_top5.pdf', plt1a, width = 4, height = 3)
```
...
```{r plotting1sfold, fig.width = 5, fig.height = 1.5}
# determine plotting order: based on ascending order of GFP/OD values (for single replicate data)

# Selecting top 5 data points + controls
sel_table6_s <- sel_table6 %>% filter(str_detect(Time, '25')) %>% ungroup() %>% mutate(category = ifelse(str_detect(category, 'pPK'), 'Mutants', as.character(category))) %>% mutate(category = fct_inorder(category), Inducer = '1 uM') %>% group_by(Samples, Inducer, category, Time) %>% filter(str_detect(Samples, 'RV01 $|rG|MG|7_21|7_6|7_28|7_32|7_17')) 

sel_table6_s$Samples %<>% fct_relevel(plotting_order_2) # manually reorder samples for plotting

sel_table2fc <- bind_rows(sel_table2_s, sel_table6_s)

plt1fc <- plot_mean_facetted(sel_table6_s) + ylab('Fold change') + scale_shape_manual(values = c(19)) #+ geom_hline(yintercept = WT_baseline, linetype = 2, alpha = .5) +  geom_hline(yintercept = rGFP_baseline, linetype = 2, alpha = .5) # plot the mean, add y axis label and a baseline indication

plt1fca <- plt1fc %>% format_classic() + scale_colour_manual(values = "black") + guides(colour = F)
plt1fca
ggsave('plots and data/S018_top5_fold change.pdf', plt1fca, width = 4, height = 2.5)
```
