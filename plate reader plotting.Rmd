...
plt7 <- plot_time_series(sel_table1_pRV, x_breaks = sheet_time_x_axis) + scale_colour_manual(values = 'black') + guides(colour = F, shape = F) + 
  annotate('rect', xmin = 8, ymin = 0, xmax = 9, ymax = Inf, alpha = .2) + 
  ggtitle(NULL)
plt7

ggsave('plate reader analysis/plots and data/S019_pRV01_2nd pulse.pdf', width = 2, height = 2)
```
...
Plotting controls, and AHL induction WT pRV01: important reporters only: GFP/OD - Change for other subsets

```{r plotting10}
sel_table1_pRV_Con <- sel_table1 %>% filter(str_detect(Samples, "Control"), str_detect(Reporter, 'GFP'), Time <= 8) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column
plt10 <- plot_time_series(sel_table1_pRV_Con, x_breaks = sheet_time_x_axis, errorbar_width = .1)
plt10
```

Plotting controls: important reporters only: GFP/OD - Change for other subsets

```{r plotting10}
sel_table1_Con <- sel_table1 %>% filter(str_detect(Samples, "Control"), str_detect(Reporter, 'GFP'), Time <= 8, str_detect(Inducer, '0')) %>% ungroup() %>% mutate(Reporter = recode_factor(Reporter, rGFP = 'Off state', fGFP = 'On state (flipped)')) # filter neccesary samples for plotting ; by default mean and variance of GFP/RFP column
plt11 <- ggplot(sel_table1_Con, aes(Time, mean, colour = Reporter)) %>%
  add_layers_time_series(x_breaks = sheet_time_x_axis, errorbar_width = .1) 
plt11
```
...
plt12 <- plot_time_series(sel_table9_fraction_flip, x_breaks = sheet_time_x_axis, y_axis_label = 'Fraction of maximum GFP/OD')
