...
Plotting GFP/OD for pRV01, pSH001 for fig 1C_v1 (individual replicates)

```{r plotting7}
metadata_columns <- c('Samples', 'Reporter', 'Time', 'Inducer', 'Replicate #')

sel_table7 <- all_days_table %>% select(all_of(metadata_columns), `GFP/OD`) %>% 
  filter(str_detect(Samples, "pRV") | str_detect(Reporter, 'pSH'), !str_detect(Inducer, '^s')) %>% 
  # filter necessary samples ; remove stationary phase induced stuff 
  
  mutate(across(Samples, ~ str_replace_all(.x, c('pRV.*' = 'Memory sensor', 
                                           'Controls' = 'Real-time sensor'))))

plt7 <-
  {ggplot(sel_table7, aes(Time, `GFP/OD`, 
                          shape = Inducer, 
                          group = interaction(Inducer, `Replicate #`))) + 
      geom_line() + geom_point(size = 2) + 
      scale_shape_manual(values = c(1,19)) +
      facet_wrap(~Samples, scales = 'free_y', ncol = 1) +  
      scale_x_continuous(breaks = c(0,6,24,48)) + 
      annotate('rect', xmin = 0, ymin = 0, xmax = 6, ymax = Inf, alpha = .2) +
      
      ylab('Fluorescence/OD (a.u.)') + ggtitle('Proof of Concept : Memory')} %>% 
  
  format_classic() %>% print () # %>% format_logscale()

ggsave('plate reader analysis/plots and data/Fig 1c_S015c.pdf', plt7, height = 3, width = 4)


# plt7b #+ scale_colour_manual(values = c('#4daf4a' ,'#e41a1c'))
# ggsave('plots and data/S015_linear.png', plt1)
# ggsave('plots and data/test.png', width = 14/3, height = 11.34/3)
```


## fig 1C_v2 : Plotting normalized GFP/OD for pRV01, pSH001 (individual replicates)

```{r plotting8}
sel_table8 <- sel_table7 %>% 
  
  group_by(Samples) %>% # group for taking mean
  mutate('Relative Fluorescence' = `GFP/OD` / max(`GFP/OD`))
  

plt8 <-
  {ggplot(sel_table8, aes(Time, `Relative Fluorescence`, 
                          shape = Inducer, 
                          group = interaction(Inducer, `Replicate #`))) + 
  ...
  format_classic() %>% print ()

ggsave('plate reader analysis/plots and data/Fig 1cv2_S015c.pdf', plt8, height = 3, width = 4.5)
...
Statistics (welch t-test --> assumes normality)

```{r stats8}
t.test(`Relative Fluorescence` ~ Inducer, # Memory significantly higher ; is obvious
       paired = T,
       alternative = 'less', 
       
       data = filter(sel_table8, Time == 48, str_detect(Samples, 'Memory')))

t.test(`Relative Fluorescence` ~ Inducer, # No difference in realtime at 48 h (0.84) ; 24 is still different (0.04)
       paired = T,
       # alternative = 'less', 
       
       data = filter(sel_table8, Time == 48, str_detect(Samples, 'Real')))

```

Statistics (Wilcoxon t-test --> paired samples) <!-- Not sure how to interpret -->

```{r stats8-nonparametric}
wilcox.test(`Relative Fluorescence` ~ Inducer, 
            paired = T,
       alternative = 'less',
