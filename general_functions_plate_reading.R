...
plot_time_series <- function(data_table, induction_duration = c(1,2), x_breaks = c(0,6,24,48), stroke_width = 1, x_axis_label = 'Time (days)', y_axis_label = 'GFP/OD (a.u.)', plot_title = 'AHL flipping with time', colour_by_var = Reporter, facet_by_var = Samples )
{
  ...
  plt <- ggplot(data_table, aes(Time, mean, colour = Reporter, shape = Inducer)) + 
    ...
    geom_errorbar(aes(ymin = mean - sd, ymax = mean + sd), width = 0.25) + facet_grid(Media ~ Samples) + geom_line() + geom_point(size = 2, fill = 'white', stroke = stroke_width) + 
