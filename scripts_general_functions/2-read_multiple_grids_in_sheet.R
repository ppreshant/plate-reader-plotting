...
#' Additional feature : recursive function to read multiple sheets 
#' Note: each sheet is processed independently. Multiple grids from each sheet along with metadata are processed
#' @param sheet_name a vector of one or more sheet names of static plate reader reads to read into the pipeline 

# TODO : Add feature to impute the Sample_name metadata from one sheet to others : Based on well name (using dplyr::fill?)
#        Take user input to activate this. For reading sample samples with and without inducer / at different time points
...
{   
  ...
  if(length(sheet_name) > 1)
  {
    sheets_df <- 
      ...
      map_dfr(set_names(sheet_name, sheet_name), # read all the grids from each sheet and join the [processed data ; baseline]
              
              read_multiple_grids_in_sheet,  # into a dataframe of columns with column names : $processed and $baseline
    ...
    # return a list of processed and baseline data
    return(list(processed = processed_data, baseline = baseline_data)) # return the combined data for all sheets, joining rows
  ...
  if(length(sheet_name) == 1)
  {
    ...
    # return the processed data and baseline data as a named list
    return(list(processed = processed.data, baseline = empty_cells_baseline))
...
# process_multiple_sheets <- function(sheet_names) # old implementation - for Archive, while testing
# {
#   sheets_df <- 
# 
#     map_dfr(sheet_names, # read all the grids from each sheet and join the [processed data ; baseline]
#             read_multiple_grids_in_sheet %>%  # into a dataframe of columns 
#               set_names(c('processed', 'baseline')) # with column names : $processed and $baseline
#     ) %>% 
#     
#     mutate(sheet_name = sheet_names, .before = 1) # add the sheetnames as another column in the master tibble
#   
#   # merging data from both sheets
#   processed_data <- bind_rows(sheets_df$processed)
#   
#   baseline_data <- 
#     sheets_df %>% group_by(sheet_name) %>% # group by sheet name
#     mutate(bs2 = map(baseline, 
#                      ~ mutate(.x, sheet_name = sheet_name))) %>% # add sheetname to baseline data
#     pull(bs2) %>% 
#     bind_rows # merge the baseline data from all sheets
#   
#   return(list(processed_data, baseline_data))
#   
# }