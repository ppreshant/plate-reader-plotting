...
plot_static_fluorescence <- function(.fluor_variable, .data = processed.data, interactive_plot = TRUE)
{
  # Parse exprs ----
  ...
  fluor_unique_mean <- fluor_order_data %>% 
  ...
  plt_fluor <- ggplot(data = fluor_order_data,
  ...
  if(interactive_plot) {plotly::ggplotly(plt_fluor) %>% plotly::style(textposition = 'top right')
    } else labelled_plot # else returns the labelled plot
